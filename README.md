...
- **claude** -- global Claude agent instructions
...
Install only one agent's instructions with `./install.sh --only codex` or
...
5. Symlink configs via Stow, including `~/.pi/`, `~/.codex/`, and `~/.claude/`
...
Most top-level directories map to `~/.config/`. Pi targets `~/.pi/`; Codex and
Claude target `~/.codex/` and `~/.claude/`.
